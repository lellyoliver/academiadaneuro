    ...
    <div class="container-plans">
        ...
        <form method="post">
            ...
            <div class="plans">
                <div class="card-plans">
                    <div style="border-bottom:1px solid rgba(0, 0, 0, 0.2);margin-bottom:20px;">
                        <h1 style="margin-bottom:20px;">Trial | Mentoria</h1>
                    </div>
                    <div class="form-plans">
                        <?php if ($product_ids): ?>
                        <div class="form-group" style="margin-bottom:0px;">
                        ...
                        <?php endif;?>
                        <div class="form-group" style="margin-bottom:0px;">
                            <label for="selected_product_ids">Mentoria 1:</label>
                            <select id="select_plan_8" name="select_plan_8" class="select-form">
                                <option>selecione o produto</option>
                                <?php
                                foreach ($product_ids as $product_id) {
                                            $selected = (get_option('_plan_mentoria_1') == $product_id) ? 'selected' : '';
                                            echo '<option value="' . esc_attr($product_id) . '" ' . $selected . '>' . esc_html(get_the_title($product_id)) . '</option>';
                                        }
                                        ?>
                            </select>
                        ...
                        <div class="form-group" style="margin-bottom:0px;">
                            <label for="selected_product_ids">Mentoria 2:</label>
                            <select id="select_plan_9" name="select_plan_9" class="select-form">
                                <option>selecione o produto</option>
                                <?php
                                foreach ($product_ids as $product_id) {
                                            $selected = (get_option('_plan_mentorias_2') == $product_id) ? 'selected' : '';
                                            echo '<option value="' . esc_attr($product_id) . '" ' . $selected . '>' . esc_html(get_the_title($product_id)) . '</option>';
                                        }
                                        ?>
                            </select>
...
<?php
    ...
    public function adn_plans_submit()
    {
        ...
        if (isset($_POST["submit"])) {
            ...
            $select_plan_1 = sanitize_text_field($_POST["select_plan_1"]);
            $select_plan_2 = sanitize_text_field($_POST["select_plan_2"]);
            $select_plan_3 = sanitize_text_field($_POST["select_plan_3"]);
            $select_plan_4 = sanitize_text_field($_POST["select_plan_4"]);
            $select_plan_5 = sanitize_text_field($_POST["select_plan_5"]);
            $select_plan_6 = sanitize_text_field($_POST["select_plan_6"]);
            $select_plan_7 = sanitize_text_field($_POST["select_plan_7"]);
            $select_plan_8 = sanitize_text_field($_POST["select_plan_8"]);
            $select_plan_9 = sanitize_text_field($_POST["select_plan_9"]);
            ...
            $plan_data = [
                ...
                "_plan_mentoria_1" => $select_plan_8,
                "_plan_mentoria_2" => $select_plan_9,
